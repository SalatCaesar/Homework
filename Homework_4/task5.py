...
# На форумах можно найти более короткое решение в три строки, я его изучил и понял, как работает,
# но дойти самому с нуля до этого не вышло, зато вышло решить задачу способом ниже самому
#   res = 0
#   for i in range(1, n + 1):  # Насколько понял тут мы решаем в обратном порядке задачу,
#        res = (res + k) % i   # перебираем от меньшего количество людей до нашего, кто выживает.
#       print(res)             # Может с опытом конечно такое в голову придёт, но не сейчас
#   print(res + 1)             # Решение основано на формуле J(n,k)=(J(n−1,k)+k)mod(n)
